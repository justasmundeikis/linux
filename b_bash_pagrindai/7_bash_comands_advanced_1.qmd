...
* [TF kanale: Įvadas - 4 video](https://youtube.com/playlist?list=PLVNXAaej57W52x2VpcEk8mA7k5yu_UCLe)

* [TF kanale: PARA metdas- 4 video](https://youtube.com/playlist?list=PLVNXAaej57W6qCYoMb_hbzDhX9et_GVlj)

* [Tiago Forte knyga  - "Building a Second Brain"](https://www.knygos.lt/lt/knygos/forte--t--building-a-second-brain-8cjaa/)
...
{{< video https://youtu.be/UeO4SV-wghs >}}
...
Atsidaryk [https://data.gov.lt/datasets/2749/](https://data.gov.lt/datasets/2749/) ir surask "Transporto priemonių valstybiniai numeriai pagal markę ir modelį" duomenis. 
...
* JSON duomenys yra organizuojami kaip „raktas-vertė“ (**key:value**) poros.
...
Štai nedidelis JSON pavyzdys, kuriame pateikiami keli asmens duomenys:
...
Šiame JSON pavyzdyje:

* „name“, „age“, „email“, ir „isStudent“ yra paprastos "raktas-vertė" poros.
* „courses“ yra masyvas (sąrašas) su keliomis vertėmis.
* „address“ yra sudėtinis objektas, kuriame yra kitos "raktas-vertė" poros.


### `jq` 

`jq` yra galingas komandinės eilutės įrankis, skirtas dirbti su JSON duomenimis Linux aplinkoje. Naudojant `jq` tu gali lengvai analizuoti, filtruoti, modifikuoti ir formatuoti JSON duomenis tiesiogiai terminale. Tai ypač naudinga dirbant su API atsakymais arba duomenų failais JSON formatu.
...
```bash
echo '[{"species":"Elnias","age":5,"weight":150,"location":"Miškas A"},{"species":"Briedis","age":8,"weight":600,"location":"Miškas B"},{"species":"Lūšis","age":3,"weight":20,"location":"Miškas C"},{"species":"Lapas","age":4,"weight":35,"location":"Miškas D"},{"species":"Lapė","age":6,"weight":15,"location":"Miškas E"}]' > zverys.json
```

Kaip matai, dabar viskas yra vienoje eilutėje, panašiai kaip ir su ValstybiniuNumeriu duomenų failu.

Naudojant `jq`, tu gali išvesti jį gražiai suformatuotą, kad būtų lengviau skaityti. `jq` komandos struktūra yra tokia: `jq <filteris> <failas>`. Paprasčiausias filtras (jokio) yra `.`. Čia `.` operatorius nurodo visą JSON duomenų struktūrą. Kai naudoji `jq .`, tu sakai, kad nori išvesti visą JSON turinį, koks jis yra, tik galbūt gražiai suformatuotą, kad būtų lengviau skaityti.
...
![](../img/bash_advanced/jq_1.png)


Jei nori išgauti tik visų žvėrių rūšis ("species"), t. y., pasirinkti vieną stulpelį, gali **filtruoti** juos:

* `.` operatorius viduje: Kaip ir ankstesniame pavyzdyje, tai yra pradinis taškas, nuo kurio pradedama navigacija per JSON struktūrą.
* `[]` operatorius: Tai yra masyvo operatorius, kuris nurodo `jq`, kad turi būti iteruojama per kiekvieną JSON masyvo elementą. Kiekvienas elementas masyve yra laikomas JSON objektu. Šiuo atveju, kiekvienas žvėris.
* species raktas: Tai yra specifinio lauko pavadinimas kiekviename masyvo objekte. Šiuo atveju tai reiškia, kad iš kiekvieno masyvo elemento (kuris yra objektas) bus išimta vertė po raktu "species".

```bash
jq '.[].species' zverys.json
```
Rezultatas:

![](../img/bash_advanced/jq_2.png)


Jei nori pasirinkti pirmą masyvą (žvėrį) iš sąrašo:


* Masyvo operatoriaus viduje `[]` reikia įrašyti konkretų numerį (**indeksą**), kuris prasideda nuo 0.
...
![](../img/bash_advanced/jq_3.png)


**Filtravimas pagal sąlygą**: Jei nori pasirinkti tik tuos žvėris, kurių svoris ("weight") yra didesnis nei 100:

```bash
jq '.[] | select(.weight > 100)' zverys.json
```

* `.` operatorius: Tai nurodo visą JSON duomenų struktūrą, pradedant nuo šaknies.
* `[]` operatorius: Šis operatorius nurodo jq, kad jis turėtų iteruoti per kiekvieną JSON masyvo elementą. Kiekvienas elementas yra JSON objektas.
* `|` operatorius: Tai yra **pipe** operatorius, kuris leidžia peradresuoti praeinantį elementą į sekančią komandos dalį. Tai veikia panašiai kaip shell komandose, leidžiant sujungti operacijas.
* `select(.weight > 100)`:
    * `select(...)` funkcija: Ji naudojama filtruoti elementus pagal tam tikrą sąlygą. Tik tie elementai, kurie atitinka sąlygą, yra išvedami į rezultatą.
    * `.weight > 100` sąlyga: Šioje vietoje, kiekvieno masyvo elemento "weight" laukas tikrinamas, ar jo vertė yra daugiau nei 100. Tik tie objektai, kurių "weight" yra didesnis nei 100, bus palikti rezultate.

![](../img/bash_advanced/jq_4.png)

Jeigu nori JSON duomenis paversti į .csv formatą, gali naudoti:


```bash
jq -r '.[] | [.species, .age, .weight, .location] | @csv' zverys.json 
```

* `jq -r`: Naudojant `-r` (arba `--raw-output`) flagą, `jq` išveda neapdorotą išvestį, kuri pašalina kabutes aplink JSON reikšmes. Tai ypač naudinga, jei išvestyje norime gauti paprastą tekstą, kurio formatas nebus sutrikdytas dvigubomis kabutėmis.
* `.` operatorius: Tai nurodo JSON šaknį ir suteikia kontekstą, nuo kurio pradedame veikti.
* `[]` operatorius: Jis nurodo, kad dirbame su JSON masyvu ir iteruojame per kiekvieną masyvo elementą. Kiekvienas elementas yra JSON objektas.
* `|` operatorius: Tai yra pipe operatorius, kurį naudojame perduoti esamą išvestį į kitą operaciją. Šioje komandoje jis naudojamas du kartus, kad kiekvienas masyvo elementas (žvėris) būtų transformuotas į specifinį formatą ir po to paverstas CSV.
* `[.species, .age, .weight, .location]`: Ši dalis suformuoja kiekvieno masyvo elemento laukus, kuriuos norime išgauti, kaip JSON masyvą. Kiekvienas elementas masyve yra paimamas iš atitinkamo JSON objekto lauko (species, age, weight, location). Tai leidžia pasirinkti tik konkrečius laukus, kuriuos norime paversti į CSV formatą.
* `@csv`: Tai yra jq operatorius, skirtas paversti JSON masyvus į CSV formatu atvaizduotas eilutes. Šis operatorius paverčia JSON masyvus į kableliu atskirtas reikšmes, t.y., CSV formatą, ir pasirūpina, kad kiekviena reikšmė būtų atitinkamai pabėgta (escaped) pagal CSV standartus.

![](../img/bash_advanced/jq_5.png)

Dabar rezultatą galima išvesti nukreipiant į failą su `>` arba naudoti tolimesniame apdorojime su kitomis bash komandomis.


::: {.callout-tip title="Praktika"}
1. Su `curl` arba `wget` parsisiųsk `https://get.data.gov.lt/datasets/gov/vulzk/registracija/Registracija`, kaip išvesties failo pavadinimą pasirink **reg_zalgiris.json**. Jei reikia, prisimink `curl` flagus su `--help` arba `tldr`.

2. Išfiltruok visus duomenis, kur registracija buvo pas **"Ortopedas"**. Pasiimk tik **gydytojo_id** ir **gydytojo_specialybe** kaip stulpelius, tada paversk filtruotą masyvą į .csv formatą ir su `sort` bei `uniq` atsakyk, pas kiek skirtingų Ortopedų buvo registracijos?

Atkeipk dėmesį, šis JSON objektas turi kelis laukus (**_data**, **_page**). 
    * **_data** yra masyvas, kuriame yra vienas ar daugiau JSON objektų, ir tavo norima informacija yra būtent čia.
    * **_page** yra kitas masyvas, kurio nenori.
    * Kadangi gydytojo_specialybe yra masyvo elemente po **_data** raktu, reikia iš pradžių patekti į šį masyvą, o tada atlikti filtravimą.

![](../img/bash_advanced/jq_6.png)

3. Identifikuok, kurie laukai iš galimų būtų prasmingi tolimesnei analizei. Paversk **reg_zalgiris.json** į csv formatą visiems pasirinktiems stulpeliams ir perleisk duomenis į failą **reg_zalgiris_from_json_to_csv.csv**.

4. Jei dabar naudosi `head`, pamatysi, jog trūksta lentelės antraščių (header). Atsidaryk ChatGPT ir suformuok, ką bandai daryti, ką gauti. Kaip atrodo tavo duomenys. Perrašyk komandas ir perkurk  **reg_zalgiris_from_json_to_csv.csv** failą.

5. Susikonspektuok pagrindinę informaciją!
...
![](../img/bash_advanced/giphy-106588401.gif)
