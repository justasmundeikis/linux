...
**BASH** (tariasi **beeš**) yra speciali kompiuterinė kalba, kurią naudojame bendraudami su terminalo programa. Ji leidžia mums rašyti ir vykdyti įsakymus arba komandas, kad galėtume valdyti kompiuterį. Panašiai kaip žmonės kalba skirtingomis kalbomis (pvz., lietuvių arba anglų), BASH yra mūsų būdas bendrauti su kompiuteriu, leidžiantis mums atlikti daug įvairių veiksmų - nuo paprastų failų valdymo iki sudėtingesnių programų vykdymo.
...
-   `echo` yra programa, kuri priima tekstą ir jį atspausdina. Ji gali atspausdinti čia, ekrane (terminale), bet gali ir atspausdinti kažkur kitur, pvz., nukreipiant tekstą į kokį nors failą. Kaip programa žino, ką spausdinti? Ji priima `argumentą`. `Argumentas` – tai, ką tu pasirenki duoti tai komandai. Šiuo atveju argumentas yra tekstas tarp abiejų kabučių `"Labas rytas"`.
...
Terminale įrašyk komandą `cat` ir perleisk jai argumentą – failo pavadinimą, kurį `cat` programa turi perskaityti! Paspausk `Enter` klavišą.
...
Nieko sau! `cat` programa perskaitė tavo failą ir jį atspausdino Terminal lange. Tau nereikėjo eiti į `Files` susirasti ir atsidaryti failo. Tu viską gali padaryti Terminal lange.

## Teksto nukreipimas į failą - `>>`

Pameni minėjau, jog `>` sukuria failą, jeigu jo nėra, o jeigu jis jau yra, tada perrašo jį. Dėl to ši programa yra gana pavojinga.
...
Na štai, išmokai naudotis terminalu, žinai aks yra komanda, argumentas. Taip pat moki nukreipti tekstą į failą. Moki pakankamai, jog dešifruoti paslaptingą žinutę!
...
Gali pasitikrinti, ar kodas.txt tikrai turi slaptą žinutę, pasinaudodama `cat` programa ir perleisdama jai argumentą `kodas.txt`.

## Šifravimo programa - `base64`

Na, o dabar belieka pasinaudoti dar viena komanda:

Programa `base64` moka koduoti ir dekoduoti. `-d` yra parametras ir reiškia dekoduoti. `kodas.txt` yra argumentas, tavo anksčiau sukurtas failas, kurio viduje yra kodas.

* **Parametras** yra papildomas nustatymas programai `base64`. Įsivaizduok programa būtų kaip įsakymas "Kepti orkaitėje", o paramtetras "nustačius 200 laispnių temperatūrą".
